...
- **Daily listing dashboard** — `MarketingService.generate_listing_report()` produces a per-listing performance dashboard (views, watchers, bids, trends). New `listing_dashboard` agent tool. `daily_listing_report_job` scheduled daily at 07:00 (configurable via `LISTING_REPORT_HOUR`).
- **Analytics** — `AnalyticsService` with `business_summary` (revenue, costs, margin, items sold), `profitability_report` (net profit per product/category/source), `inventory_report` (stock value, aging analysis), `period_comparison` (side-by-side metrics), `sourcing_analysis` (ROI per channel), `usage_report` (API token/cost tracking). Telegram `/rapport` command and weekly comparison scheduled job (Sundays 18:00).
- **Tradera category hierarchy** — DB-persisted category tree (`tradera_categories` table) with LLM-generated Swedish descriptions, bulk upsert via `sync_categories()`, fuzzy ILIKE search, auto-sync on empty DB, and `storebot-sync-categories` CLI command. `get_attribute_definitions` tool for item-type attribute lookup. Used by Listing Agent for accurate category selection.
...
- **Tests** — 852 tests across 27 modules covering db, tradera, blocket, pricing, listing, image, order, accounting, conversation, scout, marketing, analytics, postnord, CLI, retry, logging, handlers, formatting, log_viewer, compaction, model_routing, parallel, reflection, schemas, thinking, tool_filtering, and usage.
